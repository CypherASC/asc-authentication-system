...
const protecaoCSRF = require('../middlewares/protecao-csrf');
 ...
 */
router.put('/perfil', 
  process.env.NODE_ENV === 'test' ? (req, res, next) => next() : protecaoCSRF.middleware(),
  validadorMiddleware(esquemaAtualizacaoUsuario), 
  (req, res, next) => controladorUsuario.atualizarPerfil(req, res, next)
);
...
router.post('/alterar-senha', 
  process.env.NODE_ENV === 'test' ? (req, res, next) => next() : protecaoCSRF.middleware(),
  validadorMiddleware(esquemaAlteracaoSenha), 
  (req, res, next) => controladorUsuario.alterarSenha(req, res, next)
);
 ...
 */
router.delete('/sessoes/:id', 
  process.env.NODE_ENV === 'test' ? (req, res, next) => next() : protecaoCSRF.middleware(),
  (req, res, next) => controladorUsuario.encerrarSessao(req, res, next)
);
