...
router.post('/logout', 
  process.env.NODE_ENV === 'test' ? (req, res, next) => next() : protecaoCSRF.middleware(),
  (req, res, next) => controladorAuth.logout(req, res, next)
);
 ...
 */
router.get('/honeypot', 
  protecaoCSRF.middleware(),
  (req, res) => {
    const honeypot = controladorAuth.obterHoneypot(req, res);
    // Adicionar token CSRF na resposta
    if (req.csrfToken) {
      res.json({ ...honeypot, csrfToken: req.csrfToken });
    } else {
      res.json(honeypot);
    }
  }
