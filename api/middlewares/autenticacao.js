  ...
  async verificarToken(req, res, next) {
    try {
      ...
      if (payload.fingerprint && payload.fingerprint !== fingerprintAtual.fingerprint) {
        logger.seguranca('FINGERPRINT_MISMATCH', {
          nivel: 'WARN',
          usuario: payload.email,
          ip: req.ip,
          userAgent: req.headers['user-agent'],
          detalhes: {
            fingerprintEsperado: payload.fingerprint,
            fingerprintRecebido: fingerprintAtual.fingerprint
          },
          risco: 'HIGH'
        });
      ...
      logger.seguranca('TOKEN_INVALIDO', {
        nivel: 'WARN',
        ip: req.ip,
        userAgent: req.headers['user-agent'],
        detalhes: {
