...
class ControladorUsuario {
  ...
  async atualizarPerfil(req, res, next) {
    try {
      ...
      const camposPermitidos = ['nome', 'email', 'telefone', 'preferencias'];
      ...
      if (Object.keys(dadosAtualizacao).length === 0) {
        return res.status(400).json({
          sucesso: false,
          mensagem: 'Nenhum campo válido para atualização fornecido'
        });
      }

      if (dadosAtualizacao.email) {
        const usuarioExistente = await this.adaptadorBD.buscarUsuarioPorEmail(dadosAtualizacao.email);
      ...
      const usuarioAtualizado = await this.adaptadorBD.atualizarUsuario(req.usuario.id, dadosAtualizacao);
      ...
      await this.adaptadorBD.criarLogAuditoria({
        ...
        dadosAlterados: Object.keys(dadosAtualizacao),
        timestamp: new Date()
      });
